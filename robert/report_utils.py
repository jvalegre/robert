...
def get_csv_pred(suffix, path_csv_test, y_value, names, spacing):
    ...
    y_pred_sorted, y_sorted, names_sorted, sd_sorted = (
        list(t)
        for t in zip(
            *sorted(
                zip(
                    csv_test_df[f"{y_value}_pred"],
                    csv_test_df[y_value],
                    csv_test_df[names],
                    sd_list,
                ),
    ...
    count_entries = 0
    for y_val_pred, y_val, name, sd in zip(
        y_pred_sorted, y_sorted, names_sorted, sd_sorted
    ):
        ...
        if "Train:" in lines[j]:
            for k in range(j, len(lines)):
                if "Test:" in lines[k]:
                    ...
                    len(train_outliers) <= 10
                ):  # 10 outliers and the line with the % of outliers
                    if len(lines[k][6:]) > len_line:
                        outlier_line = (
                            f"{lines[k][6 : len_line + 6]}\n{lines[k][len_line + 6 :]}"
                        )
                    else:
...
def get_col_score(score_info, data_score, suffix, spacing, eval_only):
    ...
    score_title = (
        f"""&nbsp;&nbsp;·&nbsp;&nbsp;Score  {data_score[f"robert_score_{suffix}"]}"""
    )
    if suffix == "No PFI":
...
def adv_flawed(self, suffix, data_score, spacing):
    ...
    if score_flawed == 0:
        flaw_result = "The model predicts right for the right reasons."
    else:
        flaw_result = "Warning! The model probably has important flaws."
...
def adv_cv_sd(self, suffix, data_score, spacing):
    ...
    if score_cv_sd == 0:
        ...
        cv_sd_result = (
            f"Moderate variation, 4*SD = {cv_4sd} ({y_range_covered}% y-range)."
        )
    elif score_cv_sd == 2:
...
def adv_cv_diff(self, suffix, data_score, spacing, pred_type, test_set=False):
    ...
    title = "<u>3b. R² difference (model vs CV)</u>"
...
def adv_sorted_cv(self, suffix, data_score, spacing, pred_type):
    ...
    if pred_type == "reg":
        ...
        sorted_rmse = [
            f"{val}%" for val in data_score[f"scaled_{error_keyword}_sorted_{suffix}"]
        ]
    else:
...
def get_col_transpa(params_dict, suffix, section, spacing):
    ...
    excluded_params = [
        f"combined_{params_dict['error_type']}",
        "train",
        "X_descriptors",
        "y",
        "error_train",
            ...
            if ele == "model" and section == "model_section":
                ...
                if ele == "params":
                    ...
                    for param in model_params:
                        col_info += (
                            f"""{reduced_line}{param}: {model_params[param]}</p>"""
                        )
...
def calc_score(dat_files, suffix, pred_type, data_score):
    ...
    if pred_type == "reg":
        robert_score = (
            ...
            + data_score.get(f"diff_scaled_rmse_score_{suffix}", 0)
        ...
        if start_data:
            ...
            if f"Original {error_keyword.upper()} (" in line:
                ...
                if "- Sorted " in dat_verify[i + 4]:
                    sorted_cv_results = dat_verify[i + 4].split(
                        f"{error_keyword.upper()} = "
                    )[-1]
                    sorted_cv_results = ast.literal_eval(sorted_cv_results)
                    if pred_type.lower() == "reg":
                        data_score[f"scaled_{error_keyword}_sorted_{suffix}"] = [
                            round((val / data_score[f"y_range_{suffix}"]) * 100, 2)
                            for val in sorted_cv_results
                        ]
                    else:
                        data_score[f"scaled_{error_keyword}_sorted_{suffix}"] = (
                            sorted_cv_results  # no scaling for MCC
                        )

                    # define min and max values
                    data_score[f"min_scaled_{error_keyword}_{suffix}"] = min(
                        data_score[f"scaled_{error_keyword}_sorted_{suffix}"]
                    )
                    idx_min_scaled_rmse = data_score[
                        f"scaled_{error_keyword}_sorted_{suffix}"
                    ].index(data_score[f"min_scaled_{error_keyword}_{suffix}"])
                    data_score[f"max_scaled_{error_keyword}_{suffix}"] = max(
                        data_score[f"scaled_{error_keyword}_sorted_{suffix}"]
                    )
                    idx_max_scaled_rmse = data_score[
                        f"scaled_{error_keyword}_sorted_{suffix}"
                    ].index(data_score[f"max_scaled_{error_keyword}_{suffix}"])
                    ...
                    for idx, err in enumerate(
                        ...
                        if pred_type.lower() == "reg":
                            if idx == idx_min_scaled_rmse:
                                data_score[
                                    f"scaled_{error_keyword}_results_sorted_{suffix}"
                                ].append("min")
                            elif err <= (
                                data_score[f"min_scaled_{error_keyword}_{suffix}"]
                                * 1.25
                            ):
                                data_score[
                                    f"scaled_{error_keyword}_results_sorted_{suffix}"
                            ...
                            if idx == idx_max_scaled_rmse:
                                ...
                                data_score[f"max_scaled_{error_keyword}_{suffix}"]
                                * 0.75
                            ):
                                data_score[
                                    f"scaled_{error_keyword}_results_sorted_{suffix}"
    ...
    for i, line in enumerate(dat_predict):
        ...
        if start_data:
            ...
            if "o  Summary of results" in line:
                ...
                data_score[f"points_descp_ratio_{suffix}"] = dat_predict[i + 4].split()[
                    -1
                ]
                ...
                if pred_type == "reg":
                    if "-fold CV : R2 =" in dat_predict[i + 5]:
                        data_score[f"rmse_cv_{suffix}"] = float(
                            dat_predict[i + 5].split()[-1]
                        )
                        data_score[f"cv_type_{suffix}"] = " ".join(
                            [ele for ele in dat_predict[i + 5].split()[1:4]]
                        )
                        data_score[f"r2_cv_{suffix}"] = float(
                            dat_predict[i + 5].split(",")[0].split()[-1]
                        )
                    if "Test : R2 =" in dat_predict[i + 6]:
                        data_score[f"rmse_test_{suffix}"] = float(
                            dat_predict[i + 6].split()[-1]
                        )
                        data_score[f"r2_test_{suffix}"] = float(
                            dat_predict[i + 6].split(",")[0].split()[-1]
                        )
                    if "-  y range of dataset" in dat_predict[i + 8]:
                        data_score[f"y_range_{suffix}"] = float(
                            dat_predict[i + 8].split()[-1]
                        )

                    data_score[f"scaled_rmse_cv_{suffix}"] = round(
                        (
                            data_score[f"rmse_cv_{suffix}"]
                            / data_score[f"y_range_{suffix}"]
                        )
                        * 100,
                        2,
                    )
                    data_score[f"scaled_rmse_test_{suffix}"] = round(
                        (
                            data_score[f"rmse_test_{suffix}"]
                            / data_score[f"y_range_{suffix}"]
                        )
                        * 100,
                        2,
                    ...
                    data_score[f"cv_score_rmse_{suffix}"] = score_rmse_mcc(
                        pred_type, data_score[f"scaled_rmse_cv_{suffix}"]
                    )
                    data_score[f"test_score_rmse_{suffix}"] = score_rmse_mcc(
                        pred_type, data_score[f"scaled_rmse_test_{suffix}"]
                    )

                    # get penalties for R2
                    data_score[f"cv_penalty_r2_{suffix}"] = calc_penalty_r2(
                        data_score[f"r2_cv_{suffix}"]
                    )
                    data_score[f"test_penalty_r2_{suffix}"] = calc_penalty_r2(
                        data_score[f"r2_test_{suffix}"]
                    )

                    # combined scores RMSE/R2 (min 0)
                    data_score[f"cv_score_combined_{suffix}"] = (
                        data_score[f"cv_score_rmse_{suffix}"]
                        + data_score[f"cv_penalty_r2_{suffix}"]
                    )
                    if data_score[f"cv_score_combined_{suffix}"] < 0:
                        data_score[f"cv_score_combined_{suffix}"] = 0
                    data_score[f"test_score_combined_{suffix}"] = (
                        data_score[f"test_score_rmse_{suffix}"]
                        + data_score[f"test_penalty_r2_{suffix}"]
                    )
                    ...
                    data_score[f"factor_scaled_rmse_{suffix}"] = (
                        data_score[f"scaled_rmse_test_{suffix}"]
                    ...
                    pred_type == "clas"
                ):  # Process classification: using MCC extracted from CV and Test results
                    ...
                    if "5-fold" in dat_predict[i + 5]:
                        ...
                        if mcc_cv is not None:
                            data_score[f"r2_cv_{suffix}"] = (
                                mcc_cv  # storing MCC in a key keyed as r2_cv for consistency
                            )
                    ...
                    data_score[f"cv_score_rmse_{suffix}"] = score_rmse_mcc(
                        pred_type, data_score.get(f"r2_cv_{suffix}", 0)
                    )
                    data_score[f"test_score_rmse_{suffix}"] = score_rmse_mcc(
                        pred_type, data_score.get(f"r2_test_{suffix}", 0)
                    )

                    # For classification, the combined score is simply the score from MCC (no additional penalty)
                    data_score[f"cv_score_combined_{suffix}"] = data_score[
                        f"cv_score_rmse_{suffix}"
                    ]
                    data_score[f"test_score_combined_{suffix}"] = data_score[
                        f"test_score_rmse_{suffix}"
                    ]
        ...
        if os.path.exists(path_file):
            ...
            for line in datfile:
                ...
                if "Command line used in ROBERT: " in line:
                    if (
                        "--csv_name" not in command_line
                    ):  # ensures that the value for --csv_name is stored
...
def format_lines(
    module_data, max_width=122, cmd_line=False, one_column=False, spacing=""
):
    """
    ...
    for i, line in enumerate(lines):
        if "R2" in line:
            line = line.replace("R2", "R<sup>2</sup>")
        if cmd_line:
            formatted_line = textwrap.fill(
                line, width=max_width - 5, subsequent_indent=""
            )
        else:
            formatted_line = textwrap.fill(line, width=max_width, subsequent_indent="")
        if i > 0:
            formatted_lines.append(
                f'<pre style="text-align: justify;">\n{formatted_line}</pre>'
            )
        else:
            formatted_lines.append(
                f'<pre style="text-align: justify;">{formatted_line}</pre>\n'
            )
    ...
    for line in "".join(formatted_lines).split("\n"):
        if (
            line.startswith('<pre style="text-align: justify;">')
            and line != '<pre style="text-align: justify;">'
        ):
            one_col_lines += line.replace(
                '<pre style="text-align: justify;">',
                f'<pre style="text-align: justify;">{spacing * 3}',
            )
