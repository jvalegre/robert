...
class report:
    ...
    def print_score(self, dat_files, pred_type, eval_only, spacing_PFI):
        ...
        if pred_type == "clas":
            height += diff_height
        score_dat += self.print_img(
            "Results", -5, height, "PREDICT", pred_type, eval_only, diff_names=True
        )
    ...
    def print_warnings(self, pred_type, eval_only, data_score):
        ...
        # get two columns to combine and print
        (
            warnings_dict["severe_warnings_No PFI"],
            warnings_dict["severe_warnings_PFI"],
        ) = [], []
        (
            warnings_dict["moderate_warnings_No PFI"],
            warnings_dict["moderate_warnings_PFI"],
        ) = [], []
        for suffix in ["No PFI", "PFI"]:
            if eval_only and suffix == "PFI":
                ...
                warnings_dict = self.analyze_warnings(
                    data_score, suffix, warnings_dict, pred_type
                )
                ...
                if len(warnings_dict[f"severe_warnings_{suffix}"]) == 0:
                    ...
                    for sev_warning in warnings_dict[f"severe_warnings_{suffix}"]:
                        warning_print += self.print_line_warning(
                            sev_warning, style_lines, color_dict["red"], space
                        )
                ...
                if len(warnings_dict[f"moderate_warnings_{suffix}"]) == 0:
                    ...
                    for mode_warning in warnings_dict[f"moderate_warnings_{suffix}"]:
                        warning_print += self.print_line_warning(
                            mode_warning, style_lines, color_dict["yellow"], space
                        )

                # add overall assessment
                warning_print += self.print_assessment(
                    space,
                    suffix,
                    data_score,
                    style_lines,
        ...
        # page break
        warnings_dat += """<p style="page-break-after: always;"></p>"""
    ...
    def get_warning_params(self):
        ...
        space = "&nbsp;"
        color_dict = {"red": "#c56666", "yellow": "#c5c57d", "blue": "#9ba5e3"}
    ...
    def analyze_warnings(self, data_score, suffix, warnings_dict, pred_type):
        ...
        if data_score[f"flawed_mod_score_{suffix}"] < 0:
            if data_score[f"failed_tests_{suffix}"] > 0:
                warnings_dict[f"severe_warnings_{suffix}"].append(
                    "Failing required tests (Section B.1)"
                )
            else:
                warnings_dict[f"moderate_warnings_{suffix}"].append(
        ...
        # y distribution
        if (
            "WARNING! Your data is not uniform"
            in warnings_dict[f"y_dist_info_{suffix}"]
        ):
            if pred_type == "reg":
                warnings_dict[f"moderate_warnings_{suffix}"].append(
            ...
            "WARNING! Your data is slightly not uniform"
            in warnings_dict[f"y_dist_info_{suffix}"]
        ):
            if pred_type == "reg":
                warnings_dict[f"moderate_warnings_{suffix}"].append(
                ...
                warnings_dict[f"moderate_warnings_{suffix}"].append(
                    "Uneven class distribution (Section C)"
                )
        ...
        if "WARNING! High correlations" in warnings_dict[f"pearson_info_{suffix}"]:
            ...
            "WARNING! Noticeable correlations"
            in warnings_dict[f"pearson_info_{suffix}"]
        ):
            warnings_dict[f"moderate_warnings_{suffix}"].append(
    ...
    def get_warning_lines(self, pred_type):
        ...
        with open(file_pred, "r", encoding="utf-8") as datfile:
            ...
            for i, line in enumerate(lines):
                if (
                    "Ideally, variables should show low" in line
                    and not pfi_section_pearson
                ):
                    warnings_dict["pearson_info_No PFI"] = lines[i + 1][6:]
                    pfi_section_pearson = (
                        True  # the next line found will correspond to the PFI section
                    )
                elif (
                    "Ideally, variables should show low" in line and pfi_section_pearson
                ):
                    warnings_dict["pearson_info_PFI"] = lines[i + 1][6:]
                if (
                    "Ideally, the number of datapoints in" in line
                    and not pfi_section_y_dist
                ):
                    warnings_dict["y_dist_info_No PFI"] = lines[i + 2][6:]
                    pfi_section_y_dist = True
                elif (
                    "Ideally, the number of datapoints in" in line
                    and pfi_section_y_dist
                ):
...
<p style="margin-bottom: -10px; margin-top: 35px;"><strong>{space}Overall assessment</strong></p>"""
        ...
        if (
            len(warnings_dict[f"severe_warnings_{suffix}"]) > 0
            or data_score[f"robert_score_{suffix}"] < 5
        ):
            assessment_print += self.print_line_warning(
                "The model is unreliable", style_lines, color_dict["red"], space
            )

        elif data_score[f"robert_score_{suffix}"] in [9, 10]:
            if (
                pred_type == "reg"
                and len(warnings_dict[f"moderate_warnings_{suffix}"]) >= 3
            ):
                assessment_print += self.print_line_warning(
                ...
                pred_type == "clas"
                and len(warnings_dict[f"moderate_warnings_{suffix}"]) >= 2
            ):
                assessment_print += self.print_line_warning(
                    "Reliable model, but examine warnings",
                    style_lines,
                ...
                assessment_print += self.print_line_warning(
                    "The model seems reliable", style_lines, color_dict["blue"], space
                )
    ...
    def print_adv_anal(self, pred_type, eval_only, spacing_PFI, data_score):
        ...
        for section in score_sections:
            ...
            for suffix in ["No PFI", "PFI"]:
                ...
                if eval_only and suffix == "PFI":
                    ...
                    if section == "adv_flawed":
                        ...
                        columns_score.append(
                            adv_flawed(self, suffix, data_score, spacing * 2)
                        )
                        ...
                        columns_score.append(
                            adv_predict(
                                self, suffix, data_score, spacing * 2, pred_type
                            )
                        )
                        ...
                        columns_score.append(
                            adv_test(self, suffix, data_score, spacing * 2, pred_type)
                        )

                    elif section == "adv_cv_sd" and pred_type == "reg":
                        # advanced score analysis 3b, SD of CV
                        columns_score.append(
                            adv_cv_sd(self, suffix, data_score, spacing * 2)
                        )
            ...
            # add corresponding images
            section_separator = '<hr style="height: 0.5px; margin-top: 22px; margin-bottom: 0px; background-color:LightGray">'

            if section == "adv_flawed":
                height = 223
                if pred_type == "clas":
                    height -= 15
                adv_score_dat += self.print_img(
                    "VERIFY_tests", 13, height, "VERIFY", pred_type, eval_only
                )
                ...
                adv_score_dat += self.print_img(
                    "CV_variability", 10, 221, "PREDICT", pred_type, eval_only
                )
...
{style_line}4. The predictions will be shown at the end of the resulting PDF report and will be stored in the last column of two CSV files called MODEL_SIZE_test(_No)_PFI.csv, which are in the PREDICT folder.</p>"""
    ...
    def print_outliers(self, pred_type, eval_only, spacing_PFI):
        ...
        if pred_type == "reg":
            ...
            # add corresponding images
            height = 217
            outlier_dat += self.print_img(
                "Outliers", -5, height, "PREDICT", pred_type, eval_only
            )

        # add separator line and page break
        outlier_dat += '<hr style="margin-top: 20px;">'
        outlier_dat += """<p style="page-break-after: always;"></p>"""
    ...
    def print_y_distrib(self, pred_type, eval_only, spacing_PFI, warnings_dict):
        ...
        # add corresponding images
        height = 220
        distrib_dat += self.print_img(
            "y_distribution", -5, height, "PREDICT", pred_type, eval_only
        )
        ...
        for suffix in ["No PFI", "PFI"]:
            ...
            if eval_only and suffix == "PFI":
                ...
                y_distrib_sentence = format_lines(
                    warnings_dict[f"y_dist_info_{suffix}"],
                    max_width=55,
                    one_column=True,
                    spacing=spacing,
                )
        ...
        distrib_dat += """<p style="page-break-after: always;"></p>"""
    ...
    def print_features(self, warnings_dict, eval_only, spacing_PFI):
        ...
        with open(
            f"{os.getcwd()}/PREDICT/PREDICT_data.dat", "r", encoding="utf-8"
        ) as file:
            lines = file.readlines()
            linear_model_eqs = [
                lines[i + 1].strip().lstrip("- ")
                for i, line in enumerate(lines)
                if "o  Linear model equation" in line
            ]
        ...
        if linear_model_eqs:
            ...
            if len(linear_model_eqs) == 1:
                ...
                for i, eq in enumerate(linear_model_eqs):
                    if i == 0:
                        columns_eq.append(
                            f"<p style='margin-top:10px; margin-bottom:35px'>{eq}</p>"
                        )
                    else:
                        columns_eq.append(
                            f"<p style='margin-top:-5px; margin-bottom:-6px'><span style='font-weight:bold;'>{spacing * 3}Linear model equation_PFI</span></p><p style='margin-top:10px; margin-bottom:35px'>{spacing * 3}{eq}</p>"
                        )
        ...
        for _, image_pair in enumerate(image_pair_list):
            if (
                len(image_pair) < 2 and not eval_only
            ):  # Pearson graphs aren't created when >30 descriptors
                ...
                if len(image_pair) == 1:
                    ...
                    pair_list += f"{('&nbsp;') * 15}"
                    pair_list += "Pearson maps not created if >30 descriptors.</p>"
            elif eval_only:
                if len(image_pair) == 1:
        ...
        for suffix in ["No PFI", "PFI"]:
            ...
            if eval_only and suffix == "PFI":
                ...
                # split the sentence into 1 column size and add spacing line by line
                pearson_sentence = format_lines(
                    warnings_dict[f"pearson_info_{suffix}"],
                    max_width=55,
                    one_column=True,
                    spacing=spacing,
                )
        ...
        feature_dat += """<p style="page-break-after: always;"></p>"""
    ...
    def print_generate(self, pred_type, eval_only):
        ...
        if not eval_only:
            height = 236
            generate_dat += self.print_img(
                "Heatmap", -5, height, "GENERATE", pred_type, eval_only
            )
    ...
    def get_repro(self, eval_only):
        ...
        (
            ...
            total_time,
            dat_files,
        ) = repro_info(self.args.report_modules)
        ...
        if aqme_workflow:
            first_line = '<p style="text-align: justify; margin-bottom: 10px; margin-top: -16px;">'  # reduces line separation separation
        else:
            first_line = '<p style="text-align: justify; margin-bottom: 10px; margin-top: -8px;">'  # reduces line separation separation
        ...
        repro_dat += (
            f"""{reduced_line}{space}- CSV database ({self.args.csv_name})</p>"""
        )
        if self.args.csv_test != "":
        ...
        if aqme_workflow:
            ...
            if aqme_outdated:
                ...
                repro_line = []
                original_command = original_command.replace(
                    self.args.csv_name, f"AQME-ROBERT_{self.args.csv_name}"
                )
                for i, keyword in enumerate(original_command.split('"')):
                    if i == 0:
                        if (
                            "--aqme" not in keyword
                            and "--qdescp_keywords" not in keyword
                            and "--csearch_keywords" not in keyword
                        ):
                    ...
                    if i > 0:
                        if (
                            "--qdescp_keywords"
                            not in original_command.split('"')[i - 1]
                            and "--csearch_keywords"
                            not in original_command.split('"')[i - 1]
                        ):
                            if (
                                "--aqme" not in keyword
        ...
        if aqme_workflow and not aqme_updated:
            ...
            repro_dat += """<p style="text-align: justify; margin-top: -44px;"><br><strong>2. Install and adjust the versions of the following Python modules:</strong></p>"""
        else:
        ...
        # I use a very reduced line in this title because the formatted command_line comes with an extra blank line
        if aqme_workflow:
            repro_dat += """<p style="text-align: justify; margin-top: -44px;"><br><strong>4. Execution time, Python version and OS:</strong></p>"""
        else:
            repro_dat += """<p style="text-align: justify; margin-top: -37px;"><br><strong>4. Execution time, Python version and OS:</strong></p>"""
        ...
        repro_dat += """<p style="page-break-after: always;"></p>"""
    ...
    def get_transparency(self, spacing_PFI):
        ...
        transpa_dat = ""
        titles_line = '<p style="text-align: justify; margin-top: -12px; margin-bottom: 3px">'  # reduces line separation separation
        ...
        # add misc params
        transpa_dat += """<p style="text-align: justify; margin-top: -95px; margin-bottom: 3px;"><br><strong>2. ROBERT options, including prediction type (REG or CLAS), folds and repeats used for CV, etc:</strong></p>"""
    ...
    def transpa_model_misc(self, section, spacing_PFI):
        ...
        for suffix in ["No PFI", "PFI"]:
            ...
            params_dict = pd_to_dict(
                params_df
            )  # (using a dict to keep the same format of load_model)
    ...
    def get_abbrev(self):
        ...
        abbrev_dat += '<hr style="margin-top: 15px;">'

        abbrev_dat += """<p style="page-break-after: always;"></p>"""
        ...
        if csv_test_exists:
            ...
            for suffix in ["No PFI", "PFI"]:
                ...
                if eval_only and suffix == "PFI":
                    ...
                    columns_metrics.append(
                        get_csv_metrics(module_file, suffix, spacing)
                    )
            ...
            for suffix in ["No PFI", "PFI"]:
                ...
                if eval_only and suffix == "PFI":
                    ...
                    module_file = f"{os.getcwd()}/PREDICT/PREDICT_data.dat"
                    columns_pred.append(
                        get_csv_pred(suffix, path_csv_test, y_value, names, spacing)
                    )
            ...
            if len(glob.glob(f"{os.getcwd()}/PREDICT/csv_test/{prefix_img}*.png")) > 0:
                pred_dat += self.print_img(
                    prefix_img, -5, height, "PREDICT/csv_test", pred_type, eval_only
                )

            # add separator line and page break
            pred_dat += '<hr style="margin-top: 20px;">'
            pred_dat += """<p style="page-break-after: always;"></p>"""
    ...
    def module_lines(self, module, module_data, pred_type="reg", eval_only=False):
        ...
        if module == "score":
            ...
            section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">This score is designed to evaluate the models using different metrics.</i>'
        elif module == "adv_anal":
            module_name = "Section B. Advanced Score Analysis"
            section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">This section explains each component that comprises the ROBERT score. <a href="https://robert.readthedocs.io/en/latest/Report/score.html">More details here.</a></i>'
        elif module == "y_distrib":
            module_name = "Section C. Distribution of y Values"
            section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">This section shows the distribution of y values within the training and validation sets.</i>'
        elif module == "features":
            module_name = "Section D. Feature Importances"
            section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">This section presents feature importances measured using the validation set.</i>'
        elif module == "outliers":
            module_name = "Section E. Outlier Analysis"
            if pred_type == "clas":
                section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">This feature is disabled in classification problems.</i>'
            else:
                section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">This section detects outliers using the standard deviation (SD) of errors from the training set.</i>'
        elif module == "generate":
            module_name = "Section F. Model Screening"
            if eval_only:
                section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">The screening of models is disabled when using the EVALUATE module.</i>'
            else:
                section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">This section compares different combinations of hyperoptimized algorithms and partition sizes. The combined error is calculated as the product of the training error, validation error, and cross-validation error.</i>'
        elif module == "repro":
            module_name = "Section G. Reproducibility"
            section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">This section provides all the instructions to reproduce the results presented.</i>'
        elif module == "transpa":
            module_name = "Section H. Transparency"
            section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">This section contains important parameters used in scikit-learn models and ROBERT.</i>'
        elif module == "abbrev":
            module_name = "Section I. Abbreviations"
            section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">Reference section for the abbreviations used.</i>'
        elif module == "pred":
            module_name = "Section J. New Predictions"
            section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">Predictions of the external test set added with the csv_test option.</i>'
        elif module == "misc":
            module_name = "Miscellaneous"
            section_explain = '<p style="margin-top:-7px;"><i style="text-align: justify;">General tips to improve the models and instructions to predict new values.</i>'
    ...
    def print_img(
        ...
        if diff_names:
            if pred_type.lower() == "reg":
                results_images = [
                    str(file_path)
                    for file_path in module_path.rglob(f"{file_name}_*.png")
                ]
        ...
        # images with no suffixes in the names
            ...
            results_images = [
                str(file_path) for file_path in module_path.rglob(f"{file_name}_*.png")
            ]
