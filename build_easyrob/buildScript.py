    ...
    try:
        ...
        if env_exists:
            if force_recreate:
                ...
                while True:
                    response = (
                        input("Do you want to continue using it? [Y/N]: ")
                        .strip()
                        .lower()
                    )
                    ...
                    match response:
                        ...
                        case "n":
                            while True:
                                response = (
                                    input(
                                        "Do you want delete it and re-create? [Y/N]: "
                                    )
                                    .strip()
                                    .lower()
                                )
...
def conda_pack(env_name: str, output_dir: Path):
    ...
    try:
        ...
        subprocess.run(
            ["tar", "-xzf", str(output_path), "-C", str(unpacked_dir)], check=True
        )
...
def get_build_paths(root: Path) -> tuple[Path, Path, Path, Path]:
    ...
    installer_config = (
        root / "config_files" / "win32_installer.iss"
        if sys.platform == "win32"
        else None
    )
...
def parse_args():
    ...
    frontend_group.add_argument(
        "--frontend-yaml", type=Path, help="Path to the frontend environment YAML file"
    )
    ...
    backend_group.add_argument(
        "--backend-yaml", type=Path, help="Path to the backend environment YAML file"
    )
    ...
    env_group.add_argument(
        "--skip-env", action="store_true", help="Skip environment creation/verification"
    )

    # Build arguments
    build_group = parser.add_argument_group("Build Options")
    build_group.add_argument(
        "--debug", action="store_true", help="Enable debug mode in PyInstaller"
    )
    build_group.add_argument(
        "--one-file", action="store_true", help="Generate a single executable file"
    )
    build_group.add_argument(
        "--windowed", action="store_true", help="Hide console on Windows"
    )
    build_group.add_argument(
    ...
    if sys.platform == "darwin":
        parser.add_argument(
            "--version", default="0.5.0", help="Version number for macOS package"
        )
...
def main():
    ...
    try:
        ...
        if not define_env(
            args.frontend_env,
            root / "config_files" / "frontend_env.yaml",
            args.force_recreate,
        ):
            return 1
        ...
        if not define_env(
            args.backend_env,
            root / "config_files" / "backend_env.yaml",
            args.force_recreate,
        ):
            return 1
