...
def main(exe_type='command',sys_args=None):
    ...
    if not args.evaluate:
        ...
        if args.aqme:
            ...
            args.csv_name = Path(os.path.dirname(args.csv_name)).joinpath(f'AQME-ROBERT_{args.descriptor_type[0]}_{os.path.basename(args.csv_name)}')
            if args.csv_test != '':
                args.csv_test = Path(os.path.dirname(args.csv_test)).joinpath(f'AQME-ROBERT_{args.descriptor_type[0]}_{os.path.basename(args.csv_test)}')
