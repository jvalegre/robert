...
@pytest.mark.parametrize(
    "test_job",
    [
        ("t_value"),  # test for the t-value used
        ("clas"),  # test for clasification
        ("csv_test"),  # test for external test set
        ("standard"),  # standard test
        ("standard_cmd"),  # standard test
    ],
)
def test_PREDICT(test_job):
    ...
    if test_job == "standard_cmd":
        ...
        if test_job == "csv_test":
            predict_kwargs["csv_test"] = os.path.join(
                "tests", "Robert_example_test.csv"
            )

        predict(**predict_kwargs)

    # check that the DAT file is created
    assert os.path.exists(os.path.join(path_predict, "PREDICT_data.dat"))
    outfile = open(os.path.join(path_predict, "PREDICT_data.dat"), "r")
    ...
    for i, line in enumerate(outlines):
        ...
        if "o  Saving CSV databases with predictions and their SD in:" in line:
            ...
            next_line = outlines[i + 1].replace("\\", "/")
            assert "Predicted results of starting dataset: PREDICT/" in next_line
            assert "_No_PFI.csv" in next_line
        elif (
            ...
            if test_job != "clas":
                ...
                if test_job == "t_value":
                    ...
                    test_line = outlines[i + 2 + train_outliers]
                    assert "Test:" in test_line and "outliers" in test_line
                else:
                    assert train_outliers > 0
        ...
        if test_job == "clas":
            if (
                "x  WARNING! High correlations observed (up to r = 1.0 or R2 = 1.0, for x1 and x3)"
                in line
            ):
                pearson_found = True
        else:
            if (
                "x  WARNING! Noticeable correlations observed (up to r = -0.84 or R2 = 0.7, for x5 and x8)"
                in line
            ):
                pearson_found = True
        if test_job == "clas":
            ...
            if (
                "x  WARNING! Your data is slightly not uniform (Q4 has 4 points while Q1 has 12)"
                in line
            ):
                y_distrib_found = True
