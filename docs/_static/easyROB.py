...
class easyROB(tk.Tk):
    ...
    def run_robert_thread(self, command, csv_directory):
        try:
            # Change to the directory of the main CSV file before running the command
            subprocess.run(command, shell=True, cwd=csv_directory, check=True)
            ...
            # If there's an error, find the last .dat file and print the last line
            last_dat_file = self.find_last_dat_file(csv_directory)
            if last_dat_file:
                last_line = self.get_last_line_of_file(last_dat_file)
                messagebox.showwarning("Warning", f"Last line of {last_dat_file}:\n{last_line}")
