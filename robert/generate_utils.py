...
def BO_workflow(self, Xy_data, csv_df, ML_model):
    ...
    if ML_model.upper() != "MVL":
        bo_data["params"], bo_data[f"combined_{bo_data['error_type']}"] = BO_optimizer(
            self, bo_data, Xy_data
        )
        bo_data["params"] = model_adjust_params(
            self, bo_data["model"], bo_data["params"]
        )
        ...
        self.args.log.write(
            f"   o Combined {bo_data['error_type'].upper()} for {bo_data['model']} (no BO needed) (no PFI filter): {metric_combined:.2}"
        )
...
def PFI_workflow(self, csv_df, ML_model, Xy_data):
    ...
    self.args.log.write(
        f"   o Combined {PFI_dict['error_type'].upper()} for {PFI_dict['model']} (with PFI filter): {metric_combined:.2}"
    )
...
def calc_desc_keep(self, Xy_data, PFI_discard_cols):
    ...
    if desc_keep > 1:
        if self.args.pfi_max > 0:
            ...
            n_descp_PFI > 0.2 * total_points
            or n_descp_PFI >= (0.75 * desc_keep)
            or n_descp_PFI == 0
        ):
            option_one = round(0.75 * len(Xy_data["X_train_scaled"].columns))
            option_two = round(0.2 * total_points)
            option_three = round(
                len(Xy_data["X_train_scaled"].columns) - 1
            )  # for databases with two or three descriptors
...
def save_pfi_csv(self, csv_df, name_csv_hyperopt, PFI_dict, Xy_data_PFI, ML_model):
    ...
    # save the csv file
    if os.path.exists(
        self.args.destination.joinpath(f"Raw_data/PFI/{ML_model}_PFI.csv")
    ):
...
def heatmap_workflow(self, folder_hm):
    ...
    path_raw = self.args.destination.joinpath("Raw_data")
    csv_data, model_list = {}, []
