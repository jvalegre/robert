...
def get_images(module,eval_only,file=None,pred_type='reg'):
    """
    ...
    if module not in ['PREDICT','VERIFY']:
        ...
        for suffix in ['No PFI','PFI']:
            columns_score.append(get_summary(module,module_file,suffix,eval_only,titles=True,pred_type=pred_type))
    ...
    if module == 'VERIFY':
        ...
        for _,image_pair in enumerate(image_pair_list):
            html_png += text_image_pair(image_pair,eval_only)
        ...
        if csv_test_exists:
            ...
            if len(image_pair_list_test) > 0:
                for _,image_pair_test in enumerate(image_pair_list_test):
                    html_png += text_image_pair(image_pair_test,eval_only)
        ...
        html_png = ''
        if len(module_images) == 2:
            module_images = revert_list(module_images)
        html_png += text_image_pair(module_images,eval_only)
...
def text_image_pair(image_pair,eval_only):
    '''
    Get text to print a pair of images in modules sections of the report
    '''

    pair_list = ''
    if len(image_pair) > 0:
        pair_list = f'<p style="width: 91%; margin-bottom: 10px; margin-top: 10px"><img src="file:///{image_pair[0]}" style="margin: 0; width: 100%;"/>'
        if not eval_only and len(image_pair) == 2:
            pair_list += f'{("&nbsp;")*17}'
...
def get_summary(module,file,suffix,eval_only,titles=True,pred_type='reg'):
    """
    ...
    if module == 'PREDICT':
        with open(file, 'r') as datfile:
            ...
            if titles and pred_type == 'reg' and not eval_only: # only add this to the PREDICT section, not to the SCORE section
                summary.append('\n<u>Outliers (max. 10 shown)</u>\n')
                summary = summary + train_outliers + valid_outliers + test_outliers

    elif module == 'VERIFY':
        summary = []
        start_results,stop_results = None,None
        with open(file, 'r') as datfile:
            lines = datfile.readlines()
            for i,line in enumerate(lines):
                if suffix == 'No PFI':
                ...
                if suffix == 'PFI':
                    if 'o  VERIFY test values saved' in line and 'No_PFI.dat' not in line:
                        start_results = i+2
                        stop_results = i+6
            if start_results is not None and stop_results is not None:
                for line in lines[start_results:stop_results+1]:
                    if 'R2' in line:
                        line = line.replace('R2','R<sup>2</sup>')
                    if titles:
    ...
    # Column 1: No PFI
    if titles and not eval_only:
        if suffix == 'No PFI':
            column = f"""
            <p><span style="font-weight:bold;">{title_no_pfi}</span></p>
        ...
        # Column 2: PFI
            ...
            if titles:
                column = f"""
                <p><span style="font-weight:bold;">{title_pfi}</span></p>
                <pre style="text-align: justify;">{summary}</pre>
                """
...
def get_col_score(score_info,data_score,suffix,spacing_PFI,eval_only):
    """
    ...
    if suffix == 'No PFI':
        caption = f'{spacing_PFI}{title_no_pfi}'

    elif suffix == 'PFI':
        caption = f'{spacing_PFI}{title_pfi}'
