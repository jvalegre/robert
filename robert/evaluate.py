...
class evaluate:
    ...
    def __init__(self, **kwargs):
        ...
        csv_df, csv_X, csv_y = load_database(
            self, self.args.csv_name, "generate", print_info=False
        )
    ...
    def save_generate(self, csv_df, Xy_data):
        ...
        _ = csv_df.to_csv(
            f"{generate_folder}/{self.args.eval_model}_db.csv", index=None, header=True
        )
        ...
        _ = df_params.to_csv(
            f"{generate_folder}/{self.args.eval_model}.csv", index=None, header=True
        )
